...
const add = catchAsync(async (req, res, next) => {
  ...
  if (!services || !reservation) {
    return next(new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Cannot request services'));
  }
...
const requestRemoval = catchAsync(async (req, res, next) => {
  ...
  if (!result) {
    return next(new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Cannot remove service request'));
  }
...
const list = catchAsync(async (req, res, next) => {
  ...
  if (!services) {
    return next(new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Cannot get service requests'));
  }
...
const process = catchAsync(async (req, res, next) => {
  ...
  if (!result) {
    return next(new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Cannot process service requests'));
  }
...
const remove = catchAsync(async (req, res, next) => {
  ...
  if (!result) {
    return next(new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Cannot remove service requests'));
  }
